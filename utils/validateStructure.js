...
class ValidateStructure {
  ...
  validateCourses(courses) {
    ...
    let message;
    if(courses.length === 0) {
      message = `Aby móc ${this.environment} musisz uzupełnić pole dotyczące nazwy kursu`
  ...
  validateCategories(categories) {
    ...
    if(categories.length === 0) {
      message = `Aby móc ${this.environment} musisz dodać stworzyć kategorie do kursu`;
      messages.push(message)
      return messages;
    }

    if(categories.length < 2) {
      message = `Aby móc ${this.environment} musisz dodać co najmniej 2 kategorie do każdego kursu`;
      messages.push(message)
      return messages;
    }
