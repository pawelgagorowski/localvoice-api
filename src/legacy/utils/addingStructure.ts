...
class PutNewStructure {
  ...
  // adding courses
  async addCourse(courses: CoursesInStructureType, env: string) {
    const paramsForCourse: AWS.DynamoDB.DocumentClient.PutItemInput = {
      TableName: process.env.COURSES_TABLE,
      Item: {
        business: `${this.business}_@${env}`,
        list: courses.list
      }
    }
  ...
  async addCoursesToBusinessTable(courses: CoursesInStructureType, env: string) {
  ...
  // adding categories
  async addCategories (categories: ListOfCategoriesTableType[], env: string) {
  ...
  // adding lessons
  async addLessons(lessons: ListOfLessonsStructureType, env: string) {
    let key;
